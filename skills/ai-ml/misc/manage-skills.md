---
name: manage-skills
description: Discover, list, create, edit, toggle, copy, move, and delete AI agent skills across 11 tools (Antigravity, Antigravity, Agents, Windsurf, Copilot, Codex, Cline, Aider, Continue, Roo Code, Augment)
---
...
| Antigravity | `~/.cursor/skills/<name>/SKILL.md` | `.cursor/skills/<name>/SKILL.md` |
| Antigravity | `~/.gemini/skills/<name>/SKILL.md` | `.gemini/skills/<name>/SKILL.md` |
...
### Antigravity plugins (read-only)

Plugin skills are cached at `~/.cursor/plugins/cache/<org>/<plugin>/<version>/skills/<name>/SKILL.md`. These are managed by Antigravity and should not be edited directly.

## Skill File Format

For directory-based tools (Agents, Antigravity, Antigravity), skills use YAML frontmatter:
...
echo "Antigravity: $(ls ~/.cursor/skills/ 2>/dev/null | wc -l | tr -d ' ')"
echo "Antigravity: $(ls ~/.gemini/skills/ 2>/dev/null | wc -l | tr -d ' ')"
...
```bash
# For Agents/Antigravity/Antigravity (SKILL.md format)
...
```bash
# Copy from Antigravity to Antigravity
cp -r ~/.cursor/skills/my-skill ~/.gemini/skills/my-skill
...
```bash
# Search by name
find ~/.agents/skills ~/.cursor/skills ~/.gemini/skills ~/.windsurf/rules ~/.cline/rules ~/.continue/rules ~/.roo/rules -maxdepth 1 -type d 2>/dev/null | sort

# Search by content
grep -rl "search term" ~/.agents/skills/ ~/.cursor/skills/ ~/.gemini/skills/ 2>/dev/null
```

### Find disabled skills

```bash
find ~/.agents/skills ~/.cursor/skills ~/.gemini/skills -name "*.disabled" 2>/dev/null
```
...
- When copying between tools with different formats (e.g., Antigravity SKILL.md to Windsurf plain .md), adapt the file naming accordingly.
