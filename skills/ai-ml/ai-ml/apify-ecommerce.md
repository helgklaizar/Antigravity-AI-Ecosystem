...
- `.env` file with `APIFY_TOKEN` (at `~/.gemini/.env`)
...
```bash
SKILL_PATH=~/.gemini/skills/apify-ecommerce
```
...
```bash
node --env-file=~/.gemini/.env $SKILL_PATH/reference/scripts/run_actor.js \
...
| Error | Solution |
|-------|----------|
| `APIFY_TOKEN not found` | Ensure `~/.gemini/.env` contains `APIFY_TOKEN=your_token` |
