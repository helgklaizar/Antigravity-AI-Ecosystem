...
```bash
npx tsx ~/.gemini/skills/linear/scripts/setup.ts
```
...
echo 'LINEAR_API_KEY=lin_api_your_key_here' >> ~/.gemini/.env
...
```bash
npx tsx ~/.gemini/skills/linear/scripts/query.ts "query { viewer { name } }"
```
