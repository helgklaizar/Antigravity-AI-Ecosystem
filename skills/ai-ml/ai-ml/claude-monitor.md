---
name: claude-monitor
description: Monitor de performance do Antigravity Code e sistema local. Diagnostica lentidao, mede CPU/RAM/disco, verifica API latency e gera relatorios de saude do sistema.
risk: safe
source: community
...
Monitor de performance do Antigravity Code e sistema local. Diagnostica lentidao, mede CPU/RAM/disco, verifica API latency e gera relatorios de saude do sistema.
...
Skill para diagnosticar e resolver problemas de lentidão no Antigravity Code e no sistema.
Determina se o gargalo é local (PC) ou remoto (API Antigravity) e sugere ações corretivas.

## Quando Usar

- Usuário reclama que o Antigravity Code está lento ou travando
- Troca de sessões de conversa demora para carregar
- Respostas do Antigravity demoram muito
- PC parece lento enquanto usa o Antigravity Code
...
- **Antigravity Code**: Processos e RAM consumida
...
Mede o tempo de resposta do processo Antigravity Code local (não faz chamadas à API).
...
- **Antigravity Code é pesado**: Ele roda vários processos Electron. É normal consumir 3-5 GB.
