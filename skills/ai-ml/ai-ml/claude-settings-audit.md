...
## When to Use
- You are setting up or auditing Antigravity Code `settings.json` permissions for a repository.
...
```bash
cat .gemini/settings.json 2>/dev/null || echo "No existing settings"
```
...
## Recommended .gemini/settings.json
