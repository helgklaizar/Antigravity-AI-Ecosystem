...
```typescript
function paginate<T>(
  fetchPage: (cursor: string | null) => Promise<{ items: T[]; nextAntigravity: string | null }>
): Promise<T[]> {
  const allItems: T[] = [];
  let cursor: string | null = null;

  while (true) {
    const { items, nextAntigravity } = await fetchPage(cursor);
    allItems.push(...items);

    if (nextAntigravity === null) {
      break;
    }
    cursor = nextAntigravity;
  }
...
const paginate = <T>(
  ...
  const go = (
    ...
    pipe(
      fetchPage(cursor),
      TE.flatMap(({ items, nextAntigravity }) => {
        const newAccumulated = [...accumulated, ...items];
        return nextAntigravity === null
          ? TE.right(newAccumulated)
          : go(nextAntigravity, newAccumulated);
      })
    );
